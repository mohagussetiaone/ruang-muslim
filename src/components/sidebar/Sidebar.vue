...
<template>
  <div v-if="collapsible === 'none'" :class="cn('bg-secondary-background text-foreground flex h-full w-[var(--sidebar-width)] flex-col', props.class)" v-bind="$attrs">
    <slot />
  </div>
  ...
  <div v-else class="group peer hidden shrink-0 md:block" :data-state="state" :data-collapsible="state === 'collapsed' ? collapsible : ''" :data-variant="variant" :data-side="side" data-slot="sidebar">
    ...
    <div
      ...
      v-bind="$attrs"
    >
      <div data-sidebar="sidebar" data-slot="sidebar-inner" class="bg-secondary-background flex h-full w-full flex-col">
        <slot />
      </div>
    </div>
  </div>
